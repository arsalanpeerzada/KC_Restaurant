...
public class OrderListPlacedViewHolder extends GeneralViewHolder<RecyclerViewBaseAdapter.AdapterDataType> {
    ...
    public OrderListPlacedViewHolder(Context context, LiItemOrderListBinding binding, RecyclerViewBaseAdapter.EventCallback<Tap, PlacedOrderModel> callback) {
        ...
        if (itemListModel.getOrderDetail() != null) {
            for (int i = 0; i < itemListModel.getOrderDetail().size(); i++) {
                ...
                try {
                    ...
                    Toast.makeText(context, "Some Order status is Null", Toast.LENGTH_SHORT).show();
                }
